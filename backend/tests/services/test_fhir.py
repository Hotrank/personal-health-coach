...
class TestFHIRClient(unittest.TestCase):
    ...
    def test_create_update_read_patient(self):
        ...
        created_patient = self.fhir_client._create_resource(patient)
        ...
        updated_patient = self.fhir_client._update_resource(created_patient)

        self.assertEqual(updated_patient.name[0].given[0], "Jane")

        retrieved_patient = self.fhir_client.get_resource_by_id("Patient", created_patient.id)
